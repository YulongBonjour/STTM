...
def reconstruction(
    ...
    if diffusion_priors is not None:
        ...
        for diffusion_prior in diffusion_priors:
            brain_clip_embeddings0, proj_embeddings = diffusion_prior.voxel2clip(voxel.to(device).float(),sub_id=subject_id)
            if retrieve:
                continue
                ...
                if not img_variations:
                    ...
                    try:
                        brain_clip_embeddings = diffusion_prior.p_sample_loop(brain_clip_embeddings0.shape,
                                                                              ...
                                                                              cond_scale=prior_diff_cond_scale, timesteps=timesteps_prior,
                                                                              generator=generator)
                    except:
                        brain_clip_embeddings = diffusion_prior.p_sample_loop(brain_clip_embeddings0.shape,
                                                                              text_cond=dict(
                    ...
                    brain_clip_embeddings = diffusion_prior.p_sample_loop(brain_clip_embeddings0.shape,
                                                                          text_cond=dict(
                                                                              text_embed=brain_clip_embeddings0),
                                                                          cond_scale=prior_diff_cond_scale, timesteps=1000,
    ...
    for batch_ in range(n_batch):
        if recons_per_sample > 0:
            ...
            if do_classifier_free_guidance:
                input_embedding = torch.cat([torch.zeros_like(input_embedding), input_embedding]).to(device).to(
                    unet.dtype)
            ...
            if img_lowlevel is not None:  # use img_lowlevel for img2img initialization
                ...
                init_latents = vae.encode(img_lowlevel_embeddings.to(device).to(vae.dtype)).latent_dist.sample(
                    generator)
            ...
            all_recs.append(recons.unsqueeze(0))
    brain_recons = torch.cat(all_recs,dim=1)
    ...
    sims = []
    for im in range(recons_per_sample*n_batch):
        currecon = clip_extractor.embed_image(brain_recons[0, [im]].float())[-1].to(proj_embeddings.device).to(
