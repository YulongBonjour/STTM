...
class CrossBrainNetwork(nn.Module):
    ...
    def freeze_pretrained(self):
        ...
        self.input_res_block.train()
    def freeze_pretrained4prior_finetune(self):
        self.requires_grad_(False)
        self.eval()
...
class BrainDiffusionPrior(DiffusionPrior):
    ...
    def forward(
        ...
        if inference_mode:
            ...
            loss, pred = self.p_losses(image_embed * self.image_embed_scale, times, text_cond=text_cond,
            ...
            if only_loss_prior:
                ...
                return loss_prior
            if mix_up:
                loss_nce, loss_nce_global_img, loss_nce_global_txt, clip_voxels, _ = self.BiMixCo_fmri(voxel,
                                                                                                       image_embed,
                                                                                                       global_text_emb,
                                                                                                       subject_id)
                loss_prior = self.loss_prior(clip_voxels, image_embed, is_train=is_train)
                ...
                torch.cuda.empty_cache()
                # torch.cuda.empty_cache()
                clip_voxels_norm, clip_target_norm = utils.all_gather_batch_with_grad(
    ...
    def freeze_pretrained(self):
        self.requires_grad_(False)
        self.eval()
        self.voxel2clip.freeze_pretrained()
    def freeze_pretrained4prior_finetune(self):
        self.requires_grad_(True)
        self.voxel2clip.freeze_pretrained4prior_finetune()
    def train_in_proj(self, mode: bool = True):
        #print('Set for training')
        self.eval()
        self.voxel2clip.input_proj.train()
        self.voxel2clip.input_res_block.train()
    def train_prior(self):
        self.eval()
        self.net.train()
    # def BiMixCo_fmri(self,fmri,clip_hiddens,subject_id=None):
    #     voxel, perm, betas, select = utils.mixco(fmri)
    #     clip_voxels, clip_voxels_proj = self.voxel2clip(voxel, subject_id)
    #     clip_voxels_norm = nn.functional.normalize(clip_voxels_proj.flatten(1), dim=-1)
    #     clip_target_norm = nn.functional.normalize(clip_hiddens.flatten(1), dim=-1)
    ...
    #         temp=.006,
    #         perm=perm, betas=betas, select=select)
    #     return loss_nce,clip_voxels,clip_voxels_proj
    # def BiMixCo_z(self,z,clip_hiddens):
    #     z, perm, betas, select = utils.mixco(z)
    #     clip_voxels, clip_voxels_proj = self.voxel2clip.run_shared_module(z)
    #     clip_voxels_norm = nn.functional.normalize(clip_voxels_proj.flatten(1), dim=-1)
